...
gst-launch-1.0 v4l2src device=/dev/video0 ! 'image/jpeg, width=1280, height=720, framerate=30/1, format=MJPG' ! jpegdec ! nvvidconv ! 'video/x-raw(memory:NVMM), format=NV12' ! nvv4l2h264enc profile=0 maxperf-enable=1 iframeinterval=100 ! 'video/x-h264, format=NV12, stream-format=byte-stream' ! h264parse ! rtph264pay pt=96 config-interval=1 ! udpsink host=10.66.66.2 port=5000
